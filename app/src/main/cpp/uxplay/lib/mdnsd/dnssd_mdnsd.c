...
/* Android port: gethostname() on Android defaults to "localhost". When multiple
 * Android receivers run on the same LAN, they collide on "localhost.local." and
 * macOS resolvers misroute connections. Use hwaddr to guarantee unique hostnames. */
static void dnssd_make_host_name(char *dst, size_t dst_len, const char *hwaddr)
{
    char host[128];

    if (gethostname(host, sizeof(host)) || !host[0] || !strcasecmp(host, "localhost")) {
        if (hwaddr && *hwaddr) {
            snprintf(host, sizeof(host), "AirPlay-%s", hwaddr);
        } else {
...
static int dnssd_prepare_names(dnssd_private_t *dnssd, dnssd_t *dnssd_public)
{
    ...
    dnssd_make_host_name(dnssd->host_name, sizeof(dnssd->host_name), hwaddr);
    return 0;
}
