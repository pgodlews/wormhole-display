...
/* Android JNI addition: expose the ed25519 public key hex so the Java layer
 * can advertise the matching pk= in its own mDNS records. */
const char *
...
raop_stop_httpd(raop_t *raop) {
    assert(raop);
    /* Android port: raop_init2 can fail before httpd is allocated. */
